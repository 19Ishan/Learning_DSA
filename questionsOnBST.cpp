# include<iostream>
# include<queue>

using namespace std;

class Node {
    public:
        int data;
        Node* left;
        Node* right;

        Node(int val) {
            this -> data = val;
            this -> left = NULL;
            this -> right = NULL;
        }
};

Node* insertIntoBST(Node* root, int val) {
    if (root == NULL) {
        root = new Node(val);
        return root;
    }

    if (val > root -> data) {
        root -> right = insertIntoBST(root -> right, val);
    }

    else {
        root -> left = insertIntoBST(root -> left, val);
    }

    return root;
}

void createBST(Node* &root) {
    int val;
    cout << "Enter the data of the node: " << endl;
    cin >> val;

    while (val != -1) {
        root = insertIntoBST(root, val);
        // cout << "Enter the next val: " << endl;
        cout << "Enter the data of the node: " << endl;
        cin >> val;
    }
}

void levelOrderTraversal(Node* root) {
    queue<Node*> q;
    q.push(root);
    q.push(NULL);

    while (!q.empty()) {
        Node* frontNode = q.front();
        q.pop();

        if (frontNode == NULL) {
            cout << endl;
            if (!q.empty()) q.push(NULL);
        }

        else {
            cout << frontNode -> data << " ";

            if (frontNode -> left != NULL) q.push(frontNode -> left);
            if (frontNode -> right!= NULL) q.push(frontNode -> right);
        }
    }
}

void preOrderTraversal(Node* root) {
    if (root == NULL) return;

    cout << root -> data << " ";
    preOrderTraversal(root -> left);
    preOrderTraversal(root -> right);

    return;
}

void inOrderTraversal(Node* root) {
    if (root == NULL) return;

    inOrderTraversal(root -> left);
    cout << root -> data << " ";
    inOrderTraversal(root -> right);

    return;
}

void postOrderTraversal(Node* root) {
    if (root == NULL) return;

    postOrderTraversal(root -> left);
    postOrderTraversal(root -> right);
    cout << root -> data << " ";

    return;
}

Node* findMiniValue(Node* &root) {
    if (root == NULL) {
        cout << "No min value is possible: " << endl;
        return root;
    }

    Node* temp = root;

    while (temp -> left != NULL) {
        temp = temp -> left;
    }

    return temp;
}

Node* findMaxiValue(Node* root) {
    if (root == NULL) {
        cout << "No max value is possible: " << endl;
        return root;
    }

    Node* temp = root;

    while (temp -> right != NULL) {
        temp = temp -> right;
    }

    return temp;
}

bool searchInBST(Node* root, int target) {
    //Base Case
    if (root == NULL) return false;

    //Ek Case
    if (root -> data == target) return true;

    //Baaki recursion
    bool lAns = false, rAns = false;

    if (target > root -> data) rAns = searchInBST(root -> right, target);
    else lAns = searchInBST(root -> left, target);

    return lAns || rAns; 
}

 Node* deleteFromBST(Node* root, int target) {
    if (root == NULL) return NULL;

    //Start Searching
    if (target == root -> data) {
        if (root -> left == NULL && root -> right == NULL) {
            //Leaf Nodes -> Case 01
            delete root;
            return NULL;
        }

        if (root -> left != NULL && root -> right == NULL) {
            //Left Subtree is Not NULL and right subtree is NULL -> Case 02
            Node* leftSubtree = root -> left;
            delete root;
            return leftSubtree;
        }

        if (root -> left == NULL && root -> right != NULL) {
            //Left SUbtree is NULL and right subtree is Not NULL -> Case 03
            Node* rightSubtree = root -> right;
            delete root;
            return rightSubtree;
        }

        else {
            //Left and right subtrees are Not NULL -> Case 04
            Node* maxiValue = findMaxiValue(root -> left);

            //Replacement
            root -> data = maxiValue -> data;

            root -> left = deleteFromBST(root -> left, maxiValue -> data);
            return root;
        }
    }

    else if (target > root -> data) {
        root -> right = deleteFromBST(root -> right, target);
    }
    
    else {
        root -> left = deleteFromBST(root -> left, target);
    }
    return root;
}

//Question 1: Create a BST from the given inorder traversal array
Node* createBSTFromInorder(int inorder[], int s, int e) {
    //Base Case
    if (s > e) return NULL;

    int mid = s + (e - s) / 2;
    int element = inorder[mid];
    
    //Ek Case
    Node* rootNode = new Node(element);

    //Baaki recursion
    rootNode -> left = createBSTFromInorder(inorder, s, mid - 1);
    rootNode -> right = createBSTFromInorder(inorder, mid + 1, e);

    return rootNode;
}

//Question 2: LC 98: Validate Binary Search Tree
bool isValidBSTHelper(Node* root, long long int lowerBound, long long int upperBound) {
        //Base Case
    if (root == NULL) {
        return true;
    }
    //2147483647
    //Ek Case -> Current Node
    bool cond1 = ((root -> data) > lowerBound);
    bool cond2 = ((root -> data) < upperBound);
    
    bool leftAns = isValidBSTHelper(root -> left, lowerBound, root -> data);
    bool rightAns = isValidBSTHelper(root -> right, root -> data, upperBound);
    return (cond1 && cond2 && leftAns && rightAns);

}

bool isValidBST(Node* root) {
    long long int lowerBound = -2147483747; //Making it way less than the normal value of INT_MIN
    long long int upperBound = 2147483747;
     //Making it way more than the normal value of INT_MAX
    return isValidBSTHelper(root, lowerBound, upperBound);
}

//Question 3: LC 235: Lowest Common Ancestor of a Binary Search Tree
Node* lowestCommonAncestor(Node* root, Node* p, Node* q) {
    //Base Case
    if (root == NULL) return NULL;

    //Case 01 -> p and q, both lie in the left of the rootNode
    if (p -> data < root -> data && q -> data < root -> data) {
        Node* leftAns = lowestCommonAncestor(root -> left, p, q);
        return leftAns;
    }

    //Case 02 -> p and q, both lie in the right of the rootNode
    if (p -> data > root -> data && q -> data > root -> data) {
        Node* rightAns = lowestCommonAncestor(root -> right, p, q);
        return rightAns;
    }

    //Case 03 -> p lies in the left and q lies in the right
    if (p -> data < root -> data && q -> data > root -> data) {
        return root;
    } 
    //Case 04 -> p lies in the right and q lies in the left
    if (p -> data > root -> data && q -> data < root -> data) {
        return root;
    }

    return root;
}

//Question 4: LC 230 : Kth Smallest Element in a BST
int kthSmallest(Node* root, int& k) {
    //Base Case
    if (root == NULL) {
        return -1;
    }

    //L
    int leftAns = kthSmallest(root -> left, k);
    if (leftAns != -1) return leftAns;

    //N
    k--;
    if (k == 0) {
        return root -> data;
    }

    //R
    int rightAns = kthSmallest(root -> right, k);
    return rightAns;
} 

//Question 5: LC 653 : Two Sum IV - Input is a BST
void inorderTraversalHelper(Node* root, vector<int>& inorder) {
    if (root == NULL) return;
    //L
    inorderTraversalHelper(root -> left, inorder);
    //N
    inorder.push_back(root -> data);
    //R
    inorderTraversalHelper(root -> right, inorder);
    return;
} 

bool findTarget(Node* root, int k) {
    vector<int> inorder;
    inorderTraversalHelper(root, inorder);

    int s = 0, e = inorder.size() - 1;

    while (s < e) {
        int sum = inorder[s] + inorder[e];

        if (sum == k) return true;
        else if (sum > k) e--;
        else s++;
    }

    return false;
}

// Question 6: GFG : Convert BST into a sorted DLL
void bstToDLL(Node* root, Node* &head) {
    //Base Case
    if (root == NULL) {
        return;
    }

    //Ek Case mein first R -> N -> L
    //R
    bstToDLL(root -> right, head);

    //N
    root -> right = head;
    //Important Check because the head can be NULL or I mean the DLL can be NULL
    //Then no head -> left will exist
    if (head != NULL) {
        head -> left = root;
    }
    //Update head
    head = root;

    //L
    bstToDLL(root -> left, head);
    
    return;
}

void printDLL(Node* &head) {
    Node* temp = head;

    cout << "Printing the BST to DLL: " << endl;
    while (temp != NULL) {
        cout << temp -> data << " ";
        temp = temp -> right;
    }

    cout << endl;
}

int main() {

    // Node* root = NULL;
    // createBST(root);

    // levelOrderTraversal(root);

    // cout << endl;
    // cout << "Pre Order: "; 
    // preOrderTraversal(root);
    // cout << endl;

    // cout << "In Order: "; 
    // inOrderTraversal(root);
    // cout << endl;

    // cout << "Post Order: "; 
    // postOrderTraversal(root);
    // cout << endl;

    // Node* miniValue = findMiniValue(root);
    // cout << "The minimum value of the BST is: " << miniValue -> data << endl;

    // Node* maxiValue = findMaxiValue(root);
    // cout << "The maximum value of the BST is: " << maxiValue -> data << endl;

    // int target;
    // cout << "Enter the target value you want to delete: " << endl;
    // cin >> target;

    // while (target != -1) {
    //     root = deleteFromBST(root, target);
    //     cout << "The BST after deleting the target value is: " << endl;
    //     levelOrderTraversal(root);

    //     cout << "Enter the value of target: " << endl;
    //     cin >> target;
    // }

//Question 1: Create a BST from the given inorder traversal array
    // int inorder[] = {10, 20, 30, 40, 50, 60, 70};
    // int size = 7;

    // int s = 0;
    // int e = size - 1;

    // Node* root = createBSTFromInorder(inorder, s, e);
    // levelOrderTraversal(root);

//Question : BST to DLL
    int inorder[] = {10, 20, 30, 40, 50, 60, 70};
    int size = 7;
    int s = 0, e = size - 1;
    Node* root = createBSTFromInorder(inorder, s, e);
    levelOrderTraversal(root);

    Node* head = NULL;
    bstToDLL(root, head);

    printDLL(head);

    return 0;
}

//50 30 40 20 60 55 70 80 25 -1